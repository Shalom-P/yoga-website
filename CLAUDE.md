...
- **`meet-retry`** (~15–30 min) — runs Meet in **both** directions, batched (50/run) to avoid Calendar rate limits. Forward: retries `createMeetEvent` for not-yet-started sessions stuck at `meet_status='pending'|'failed'`. Reverse: deletes the event for sessions at `meet_status='release_pending'` and clears the row to `NULL` (**not** `'failed'`, which the forward pass would immediately re-provision). That mark is written by the `cancel-booking` Edge Function, which the native iOS app calls and which cannot delete the event itself — Vercel OIDC is not mintable from Deno. Migration `0038`.
