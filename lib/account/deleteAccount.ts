// Single implementation of "permanently delete a user account and everything
// that must go with it". Both deletion entry points call this:
//   * POST /api/account/delete        — the customer deleting themselves
//                                       (App Store Guideline 5.1.1(v))
//   * DELETE /api/admin/customers/:id — an admin deleting someone else
...
/** Why a deletion was refused or failed. Callers map these to their own copy. */
export type DeleteAccountFailure =
  /** Role could not be established — we fail closed rather than guess. */
  | "role_check_failed"
  /** Target is a teacher or admin; those must be demoted/unlinked first. */
  | "role_managed_account"
  /** The Admin API delete itself failed. Nothing irreversible happened. */
  | "delete_failed";

export type DeleteAccountResult =
  | { ok: true; deleted: AccountSnapshot }
  | { ok: false; reason: DeleteAccountFailure };

/** What the account looked like just before deletion — for audit logging. */
...
/**
 * Permanently delete `userId`. Order matters:
 *
 ...
 *
 * @param cancellationReason stamped on bookings cancelled by the deletion, so
 *        the two entry points stay distinguishable in the booking history.
 * @param logPrefix console tag identifying the calling route.
 */
export async function deleteAccountCascade(
  userId: string,
  {
    cancellationReason,
    logPrefix,
  }: { cancellationReason: string; logPrefix: string },
  ...
  if (profileError || !profile) {
    console.error(`${logPrefix} role lookup failed:`, profileError?.message);
    return { ok: false, reason: "role_check_failed" };
  }
  if (profile.role === "teacher" || profile.role === "admin") {
    return { ok: false, reason: "role_managed_account" };
  }
  ...
  try {
    ...
    console.error(`${logPrefix} future-session cleanup failed:`, err);
  ...
  try {
    ...
    console.error(`${logPrefix} bank-transfer settlement failed:`, err);
  ...
  if (error) {
    ...
    console.error(`${logPrefix} deleteUser failed:`, error.message);
  ...
  try {
    ...
    console.error(`${logPrefix} storage cleanup failed:`, err);
